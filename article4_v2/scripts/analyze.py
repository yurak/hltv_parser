...
Runs all quantitative experiments, writes reproducible numerical outputs to
  ../outputs/tables/*.csv   ../outputs/figures/*.png   ../outputs/logs/analysis_log.txt

All claims in the paper trace back to artifacts produced here.
Run from the project root or from scripts/: paths resolve relative to the repo.
"""
...
# DATA / OUT_DIR can be overridden via env vars so the same pipeline runs on a
# second dataset (e.g. IEEE-CIS) without clobbering the primary outputs:
#   FRAUD_DATA=data/raw/df_ieeecis.csv FRAUD_OUT=outputs_ieeecis python scripts/analyze.py
DATA = Path(os.environ.get("FRAUD_DATA", ROOT / "data" / "raw" / "df.csv"))
OUT_DIR = Path(os.environ.get("FRAUD_OUT", ROOT / "outputs"))

# LOG is opened lazily on first log() call so that importing this module
# (e.g. from build_figures.py) does not truncate the analysis log.
LOG = None
...
def log(msg: str) -> None:
    ...
    if LOG is None:
        ...
        LOG = (OUT_DIR / "logs" / "analysis_log.txt").open("w", encoding="utf-8")
    print(msg)
    LOG.write(msg + "\n")
    LOG.flush()
...
def load_and_engineer() -> pd.DataFrame:
    ...
    # An adapter (load_external.py) may supply a precomputed BIN/issuer column;
    # otherwise derive it from the masked card number as in the primary dataset.
    if "BIN" not in df.columns:
        df["BIN"] = df["Card Number"].astype(str).str[:6]
...
def main() -> None:
    ...
    for col, k in [("BIN", 20.0), ("Amount", 10.0)]:
        ...
        feat = f"{col.lower()}_te" if col == "BIN" else "amount_te"
        df[feat] = df[col].map(enc.to_dict()).fillna(prior_full).astype(float)
